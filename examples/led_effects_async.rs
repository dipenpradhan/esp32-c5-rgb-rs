...
///
/// Uses the async RMT channel: `transmit` arms the channel's end-of-frame
/// interrupt and the returned future resolves when hardware finishes the
/// frame (~330 µs, dominated by the 300 µs reset), so the executor is free to
/// run other tasks during the transfer instead of a blocking `wait` spinning
/// the CPU. The channel is `&mut` in async mode (no ownership handoff), so it
/// is simply mutated in place.
async fn ws2812_rgb_rmt<'a>(channel: &mut Channel<'a, Async, Tx>, tick_hz: u32, rgb: &[u8; 3]) {
    let frame = encode_rgb_v2(rgb);
    let codes = frame_codes(&frame, tick_hz);
    channel.transmit(&codes).await.unwrap()
}
...
async fn main(_spawner: embassy_executor::Spawner) -> ! {
    ...
    // needed for the four WS2812 phase widths (400/850/800/450 ns). Async mode
    // is required for the interrupt-driven `transmit().await` below.
    let rmt = Rmt::new(peripherals.RMT, Rate::from_mhz(RMT_SCLK_MHZ))
        .unwrap()
        .into_async();
        ...
        for effect in &config.effects {
            match effect {
                LedEffect::Blink {
                    ...
                    for &color in colors {
                        ws2812_rgb_rmt(&mut channel, tick_hz, &color).await;
